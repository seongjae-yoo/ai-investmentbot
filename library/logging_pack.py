...
# AiInvestmentbot.log 라는 이름으로 로그파일이 만들어진다.

# log파일 위치와 로그 이름을 설정한다. (촬영 후 아래 수정 하였습니다.)
file_path = pathlib.Path(__file__).parent.parent.absolute() / 'log' / 'AiInvestmentbot.log'
...
# 시뮬레이터에 있는 모든 print를 ctrl + shift + r -> scope -> Current File 설정 후
