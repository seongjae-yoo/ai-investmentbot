...
from ai.angular_grad import AngularGrad

from ai.attention_3d_block.attention_3d_block import attention_3d_block2
...
#model = sweep_model()
#model = wandb_sweep_train()
# 학습 시작


#sweep_model = sweep_model()  

#  history = wandb_sweep_train_version4(shuffled_data, sweep_model, EPOCHS, BATCH_SIZE, verbose=1) 
history = wandb_sweep_train_version5(shuffled_data, sweep_model, EPOCHS, BATCH_SIZE, verbose=1)  # wandb_sweep_train 함수명 바꾸면 CNN_Attention_BiLSTM 모델 sweep 실험할 수 있음
...
#     x = Conv1D(filters = wandb_sweep_train.config.Conv1D_units,padding='valid' ,kernel_size = 1,strides=wandb_sweep_train.config.Conv1D_strides)(input_layer)
...
#     lstm_out = tf.keras.layers.Bidirectional(LSTM(wandb_sweep_train.config.BiLstm_units,kernel_initializer = kernel_initializer, recurrent_initializer=recurrent_initializer, bias_initializer=bias_initializer,return_sequences=True),name='bilstm')(attention_mul)
#     lstm_out = Dropout(wandb_sweep_train.config.dropout)(lstm_out)
#     x = Flatten()(lstm_out)
...
# future_price = sweep_predict(data, sweep_model, n_steps=N_STEPS)
