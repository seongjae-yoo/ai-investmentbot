...
from ai.SPPModel import create_model_GRU, load_data, create_model, evaluate, predict, DataNotEnough, create_model_Bidirectional
...
def filtered_by_basic_lstm(dataset, ai_settings):
    ...
    #                         n_layers=ai_settings['n_layers'], dropout=ai_settings['dropout'])
    model = create_model_GRU(n_steps=ai_settings['n_steps'], loss=ai_settings['loss'], units=ai_settings['units'],
                            n_layers=ai_settings['n_layers'], dropout=ai_settings['dropout'])
    # model = create_model_Bidirectional(n_steps=ai_settings['n_steps'], loss=ai_settings['loss'], units=ai_settings['units'],
    #                         n_layers=ai_settings['n_layers'], dropout=ai_settings['dropout'])

    early_stopping = EarlyStopping(monitor='val_loss', patience=80)  # 200번이상 더 좋은 결과가 없으면 학습을 멈춤
