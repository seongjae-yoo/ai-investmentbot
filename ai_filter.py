...
from ai.SPPModel import load_data, predict, DataNotEnough, CNN_Attention_BiLSTM_Version3,CNN_Attention_BiLSTM_Version11, CNN_Attention_BiLSTM_Version17,CNN_Attention_BiLSTM_Version9, CNN_Attention_BiLSTM_Version27, BiGRU_CNN_BiLSTM_Attention_version2,BiLSTM_Attention_CNN_version2, CNN_BiLSTM_Attention_version2 
...
def ai_filter(ai_filter_num, engine, until=datetime.datetime.today()):
    ...
    #CNN_Attention_BiLSTM_Version3(maxlen=5, units=21, dropout=0.3, n_steps=1, LOSS = "mae", optimizer= 'cos'):
            ...
            ai_settings = {   
                        ...
                        "ratio_cut": 1,
                        "table": "daily_craw",
                        "is_used_predicted_close" : True #false는 마이너스
                    }
            ...
            for code_name, in buy_list:
                ...
                try:
                    if 1<= len(df) <=5000:
                        ai_settings['model'] = BiGRU_CNN_BiLSTM_Attention_version2()  # 셀트리온헬스케어 model-best.h5 -> pretty-sweep-110 사용
                        filtered = filtered_by_basic_lstm(df, ai_settings)
                    elif len(df) >5000:       
                        ai_settings['model'] = BiGRU_CNN_BiLSTM_Attention_version2()
                        filtered = filtered_by_basic_lstm_v2(df, ai_settings)       
                except (DataNotEnough, ValueError):
                    print(f"테스트 데이터가 적어서 realtime_daily_buy_list 에서 제외")
                    filtered_list.append(code_name)
            ...
            for code_name, in buy_list:
                ...
                try:
                    if 1<= len(df) <=5000:
                        ai_settings['model'] = BiLSTM_Attention_CNN_version2()  
                        filtered = filtered_by_basic_lstm(df, ai_settings)
                    elif len(df) >5000:       
                        ai_settings['model'] = BiLSTM_Attention_CNN_version2()
                        filtered = filtered_by_basic_lstm_v2(df, ai_settings)       
            ...
            for code_name, in buy_list:
                ...
                try:
                    if 1<= len(df) <=5000:
                        ai_settings['model'] = CNN_BiLSTM_Attention_version2()  
                        filtered = filtered_by_basic_lstm(df, ai_settings)
                    elif len(df) >5000:       
                        ai_settings['model'] = CNN_BiLSTM_Attention_version2()
                        filtered = filtered_by_basic_lstm_v2(df, ai_settings)       
